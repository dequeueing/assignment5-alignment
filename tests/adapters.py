...
def run_tokenize_prompt_and_output(
    ...
    """
    from cs336_alignment.fine_tuning import tokenize_prompt_and_output
...
def run_compute_entropy(logits: torch.Tensor) -> torch.Tensor:
    ...
    from cs336_alignment.fine_tuning import compute_entropy
...
def run_get_response_log_probs(
    ...
    """
    from cs336_alignment.fine_tuning import get_response_log_probs
...
def run_compute_naive_policy_gradient_loss(
    ...
    """
    # Policy gradient loss: -A * log p
    # Broadcast advantage over sequence dimension: (batch_size, 1) -> (batch_size, sequence_length)
...
def run_masked_mean(tensor: torch.Tensor, mask: torch.Tensor, dim: int | None = None) -> torch.Tensor:
    ...
    """
    # Apply mask to tensor (set non-masked positions to 0)
    masked_tensor = tensor * mask

    if dim is None:
        # Sum all elements and divide by count of masked elements
        return masked_tensor.sum() / mask.sum()
    else:
        # Compute sum over the dimension
        sum_over_dim = masked_tensor.sum(dim=dim)
        # Count non-zero mask elements along the dimension
        count = mask.sum(dim=dim)
        return sum_over_dim / count
...
def run_sft_microbatch_train_step(
    ...
    """
    from cs336_alignment.fine_tuning import sft_microbatch_train_step
...
def run_masked_normalize(
    ...
    """
    from cs336_alignment.fine_tuning import masked_normalize
